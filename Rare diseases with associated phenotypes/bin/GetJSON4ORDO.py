#!/usr/bin/env python
import math,os,os.path,sys,json,datetime,time,shutil
...
def Get_OldORDOs():
   ...
   for afile in os.listdir('/env/orphanet/pub/www/orphadata/data/ORDO/'):
      if afile.startswith('ordo_orphanet_') is True and afile.endswith('.zip'):
         ...
         aTimeList = time.ctime(os.path.getctime('/env/orphanet/pub/www/orphadata/data/ORDO/'+ afile)).split()
         dico_ordo['aDate'] = ("%s %s %s")%(aTimeList[2], aTimeList[1] , aTimeList[-1][-2:])
...
def Get_LastORDOs():
      ...
      if afile.startswith('ORDO_') is True and afile.endswith('.owl'):
         ...
         for aLang in dico_lang.keys():
            if aLang == afile.split('_')[1]:
               dico_ordo['lang'] = dico_lang[aLang]
         ...
         aTimeList = time.ctime(os.path.getctime('/env/orphanet/pub/www/orphadata/data/ORDO/' + afile)).split()
         dico_ordo['aDate'] = ("%s %s %s")%(aTimeList[2], aTimeList[1] , aTimeList[-1][-2:])
         dico_ordo['aNote'] = 'http://www.orphadata.org/data/ORDO/ORDO_releaseNotes'+ dico_ordo['aVersionordo'] + '.txt'
   ...
   for anORDO in listordo_tmp:
      if anORDO["lang"] == "English":
         shutil.copy("/env/orphanet/pub/www/orphadata/data/ORDO/ORDO_en_" + anORDO["aVersionordo"] + ".owl" , "ordo_orphanet.owl")
